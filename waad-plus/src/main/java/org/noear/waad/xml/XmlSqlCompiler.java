...
public class XmlSqlCompiler {
    ...
    public static String parse(URL xmlFile) throws Exception{
        ...
        String classname = namespace.substring(sepindex+1); //.replace(".","_"); //namespace.replace(".","_"); //"waad_xml_sql";
        ...
        sb.append("import org.noear.waad.SQLBuilder;\n");
    ...
    private static Document parseDoc(URL xmlFile) throws Exception{
        if(docBf ==null) {
            ...
            docB.setEntityResolver((publicId, systemId) -> {
                if (systemId.contains("waad-mapper.dtd")) {
                    InputStream dtdStream = XmlSqlBlock.class.getResourceAsStream("/org/noear/waad/xml/waad-mapper.dtd");
                    return new InputSource(dtdStream);
                    //return new InputSource(new StringReader(""));
                } else if (systemId.contains("waad-generator.dtd")) {
                    InputStream dtdStream = XmlSqlBlock.class.getResourceAsStream("/org/noear/waad/xml/waad-generator.dtd");
                    return new InputSource(dtdStream);
                    //return new InputSource(new StringReader(""));
                } else {
                    return null;
    ...
    private static void parseForNode(StringBuilder sb,String sqlBuilderName, XmlSqlBlock dblock, Node n , int depth) {
         ...
         *     Iterator<waaddemo.mapper.UserModel> m_iterator = list.iterator(); //@items配置
         *     while (m_iterator.hasNext()) {
         *         waaddemo.mapper.UserModel m = m_iterator.next(); //@var配置
         *
